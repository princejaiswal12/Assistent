...
const BUILTIN = {
  ...
  wikipedia: "https://www.wikipedia.org", linkedin: "https://www.linkedin.com", reddit: "https://www.reddit.com",
  amazon: "https://www.amazon.com", stackoverflow: "https://stackoverflow.com", chatgpt: "https://chatgpt.com",
};

function clean(text) { return text.trim().replace(/\s+/g, " "); }
function stripPrefix(text, prefixes) {
  for (const prefix of prefixes) if (text.toLowerCase().startsWith(prefix.toLowerCase())) return text.slice(prefix.length).trim();
  return text;
}

export function parseIntent(input, { engine = "google", websites = {} } = {}) {
  const original = clean(input), text = original.toLowerCase();
  ...
  if (siteSearch) {
    ...
    if (base) return { type: siteName.toUpperCase(), query, url: buildWebsiteSearchUrl(base, query), label: `${siteName}: ${query}` };
  }
  ...
  if (openMatch) {
    const name = openMatch[2].trim().toLowerCase(), base = sites[name];
