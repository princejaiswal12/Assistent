  ...
  return (
    <div className="modal-backdrop" onMouseDown={onClose}>
      <aside className="settings-panel" onMouseDown={(e) => e.stopPropagation()}>
        ...
        <label>
          Search engine
          <select value={settings.engine} onChange={(e) => update({ ...settings, engine: e.target.value })}>
        ...
        <label>
          Default action
          <select value={settings.defaultAction} onChange={(e) => update({ ...settings, defaultAction: e.target.value })}>
            <option value="search">Search web</option>
            <option value="confirm">Ask for confirmation</option>
            <option value="ignore">Ignore unknown commands</option>
          </select>
        </label>

        <label>
          Duplicate suppression
          <select value={settings.duplicateWindow} onChange={(e) => update({ ...settings, duplicateWindow: Number(e.target.value) })}>
        ...
        <div className="custom-sites">
          ...
          <p className="muted">Say “open [name]” after adding a site.</p>
        ...
        <div className="privacy-box">
          ...
          <p>Microphone access starts only after you activate listening. This app does not upload or store raw microphone audio. Recognized text stays in this page unless you clear it.</p>
        </div>
      </aside>
    </div>
  );
